...
try:
    ...
    # 1. Scroll directly to #videos section (modules grid is gone)
    call_cdp(ws, "Runtime.evaluate", {"expression": "document.getElementById('videos').scrollIntoView({behavior: 'instant'});"})
    time.sleep(1)
    shoot("final_01_videos_section")

    # 2. Open video modal
    call_cdp(ws, "Runtime.evaluate", {"expression": "openVideoModal(1);"})
    time.sleep(0.8)
    shoot("final_02_video_modal_with_guide_btn")

    # 3. Click curriculum guide from video modal
    call_cdp(ws, "Runtime.evaluate", {"expression": "openModuleGuideFromVideo();"})
    time.sleep(0.8)
    shoot("final_03_module_guide_modal")
